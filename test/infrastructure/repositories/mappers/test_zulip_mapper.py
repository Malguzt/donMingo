...
class TestZulipMapper:
    ...
    def test_should_map_zulip_message_to_chat_message(self, mock_datetime_class, mock_user_class, mock_chat_message_class):
        # Setup mocks
        mock_user = Mock()
        mock_user_class.return_value = mock_user
        
        mock_chat_message = Mock()
        mock_chat_message_class.return_value = mock_chat_message
        ...
        mock_datetime_class.fromtimestamp.assert_called_once_with(1609459200)
        
        # Should create ChatMessage with correct parameters
        mock_chat_message_class.assert_called_once_with(
            id=12345,
            content="Hello, how are you?",
            sender=mock_user,
    ...
    @patch('infrastructure.repositories.mappers.zulip_mapper.User')
    @patch('infrastructure.repositories.mappers.zulip_mapper.datetime')
    def test_should_handle_missing_fields_in_zulip_message(self, mock_datetime_class, mock_user_class, mock_chat_message_class):
        # Setup mocks
        mock_user = Mock()
        mock_user_class.return_value = mock_user
        ...
        mock_datetime_class.fromtimestamp.assert_called_once_with(None)
