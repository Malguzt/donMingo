...
class TestZulipConfig:
    ...
    def test_should_raise_error_when_all_environment_variables_missing(self):
        with pytest.raises(ValueError, match="Missing required environment variables: ZULIP_API_KEY, ZULIP_EMAIL, ZULIP_SITE"):
            ZulipConfig()
    ...
    def test_should_raise_error_when_some_environment_variables_missing(self):
        with pytest.raises(ValueError, match="Missing required environment variables: ZULIP_EMAIL, ZULIP_SITE"):
            ZulipConfig()
    ...
    def test_should_raise_error_when_only_site_missing(self):
        with pytest.raises(ValueError, match="Missing required environment variables: ZULIP_SITE"):
            ZulipConfig()

    @patch.dict(os.environ, {"ZULIP_API_KEY": "", "ZULIP_EMAIL": "test@example.com", "ZULIP_SITE": "test.zulipchat.com"}, clear=True)
    def test_should_raise_error_when_environment_variable_is_empty_string(self):
        with pytest.raises(ValueError, match="Missing required environment variables: ZULIP_API_KEY"):
            ZulipConfig()
